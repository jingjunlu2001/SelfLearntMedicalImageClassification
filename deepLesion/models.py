...
def dense_with_bottleneck(input_mat, kernel_dim, dilation, growth_rate):
    if not (dilation):
        ...
        a = Conv2D(growth_rate, kernel_size=(kernel_dim, kernel_dim), strides=(1, 1), padding='same',
                   dilation_rate=dilation)(input_mat)
        a = Activation('elu')(a)
...
def Unet2(input_img, n_filters=16, dropout=0.2, batch_norm=True):
    c1 = conv_block(input_img, n_filters, 3, batch_norm)
    p1 = MaxPooling2D(pool_size=(2, 2), strides=2)(c1)
    p1 = Dropout(dropout)(p1)
    ...
    c5 = conv_block(p4, n_filters * 16, 3, batch_norm)
    c5 = Conv2D(16, kernel_size=(3, 3), strides=(1, 1), padding='same')(c5)
    c5 = Activation('relu')(c5)
    c5 = conv_block(c5, n_filters * 16, 3, batch_norm)
    ...
    c9 = conv_block(u9, n_filters, 3, batch_norm)
    outputs = Conv2D(100, (1, 1), activation='sigmoid')(c9)
