...
for index, row in patient_df.iterrows():
    ...
    for (start_x, start_y, end_x, end_y) in row['bbox']:
        lesion_center_x.append(int((start_x+end_x)//2))
        lesion_center_y.append(int((start_y+end_y)//2))

x_raw = np.asarray(img)
x_raw = x_raw.reshape(409, 512, 512, 3)
x = np.zeros((409, 224, 224, 3))
for i in range(409):
    if 112 <= lesion_center_x[i] <= 400 and 112 <= lesion_center_y[i] <= 400:
        x[i, :, :, :] = x_raw[i, (lesion_center_x[i]-112):(lesion_center_x[i]+112), (lesion_center_y[i]-112):(lesion_center_y[i]+112), :]
    elif lesion_center_x[i] < 112 and 112 <= lesion_center_y[i] <= 400:
        x[i, (112-lesion_center_x[i]):, :, :] = x_raw[i, :(lesion_center_x[i]+112), (lesion_center_y[i]-112):(lesion_center_y[i]+112), :]
    elif lesion_center_x[i] >400 and 112 <= lesion_center_y[i] <= 400:
        x[i, :(624-lesion_center_x[i]), :, :] = x_raw[i, (lesion_center_x[i]-112):, (lesion_center_y[i]-112):(lesion_center_y[i]+112), :]
    elif 112 <= lesion_center_x[i] <= 400 and lesion_center_y[i] < 112:
        x[i, :, (112-lesion_center_y[i]):, :] = x_raw[i, (lesion_center_x[i]-112):(lesion_center_x[i]+112), :(lesion_center_y[i]+112), :]
    elif 112 <= lesion_center_x[i] <= 400 and lesion_center_y[i] > 400:
        x[i, :, :(624-lesion_center_y[i]), :] = x_raw[i, (lesion_center_x[i]-112):(lesion_center_x[i]+112), (lesion_center_y[i]-112):, :]
x = x / 2048
